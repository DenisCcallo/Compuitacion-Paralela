...
int main(int argc, char* argv[]) {
   ...
   for (thread = 0; thread < thread_count; thread++) {
      ...
      sem_init(&semaphores[thread], 0, 0); //inicia semaforos
   }
...
void Usage(char* prog_name) {
   ...
   fprintf(stderr, "use: %s <numero de hilos>\n", prog_name);
   exit(0);
}
...
void *Send_msg(void* rank) {
   ...
   sem_post(&semaphores[dest]);  // "desbolquea" 

   sem_wait(&semaphores[my_rank]); //espera a que el semaforo se desbloquee
